...
class ProfileScreen extends StatelessWidget {
  final User user; // Thêm biến user vào constructor

  ProfileScreen({required this.user}); // Nhận user từ constructor
              ...
              Row(
                children: [
                  CircleAvatar(
                    radius: 40,
                    backgroundImage:
                        NetworkImage(user.profileImageUrl), // Dùng ảnh từ user
                  ),
                  SizedBox(width: 16),
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
...
class User {
  final String name;
  final String email;
  final String password; // Thêm password vào model User
  final String profileImageUrl;

  User({
    required this.name,
    required this.email,
    required this.password, // Cập nhật constructor
    required this.profileImageUrl,
  });
}

// Danh sách người dùng đã đăng ký (Giả sử lưu trong bộ nhớ tạm thời)
List<User> users = [];
...
class _MainScreenState extends State<MainScreen> {
  ...
  void initState() {
    super.initState();
    // Khởi tạo _screens khi build
    _screens = [
      Explorescreen(),
      MyTripsScreen(),
      ChatScreen(),
      ProfileScreen(user: widget.user), // Sử dụng widget.user
    ];
  }
